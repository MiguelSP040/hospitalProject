...
public class BedService {
    ...
    @Transactional(readOnly = true)
    public ResponseEntity<?> findOccupiedBedsByPatient(long patientId) {
        List<Bed> list = new ArrayList<>();
        String message = "";
        if(bedRepository.findOccupiedBedsByPatient(patientId).isEmpty()){
            message="No se encontraron camas ocupadas por este paciente";
        } else {
            for (Bed b: bedRepository.findOccupiedBedsByPatient(patientId)){
                list.add(b);
            }
            message="Operacion exitosa";
        }
